...
modis.download = function( product, month, year, h, v, ver, username="", pwd="", tmp.dir="./") {
  ...
  download.file(url,list.fname,method="wget",quiet=TRUE,extra=extra)
  ...
  pattern = paste("href=\"",year,".",addZero(month),sep="")
  ...
  for ( d in 1:length(valid) ) {
    ...
    # Download the file
    #
    download.file(url.month,day.fname,method="wget",quiet=TRUE,extra=extra)
...
#######################
#The following parts are created by Yanni Zhan for different purposes
